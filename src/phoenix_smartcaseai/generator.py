...
class StoryBDDGenerator:
    ...
    def _generate_with_single_provider(self, provider_name: str, llm, user_story: str, output_format: str, additional_context: str = "") -> Dict:
        ...
        try:
            # Get raw response from LLM with timeout protection
            # Use asyncio timeout if available, otherwise use regular invoke
            try:
                ...
                # If invoke fails, try with a simpler approach
                raise Exception(f"LLM invocation failed: {str(invoke_error)}")
            ...
            try:
                ...
                # Fallback: Try to handle the case where LLM returns {"items": [...]} or other wrapped formats
                try:
                    ...
                    if isinstance(parsed_json, dict) and "items" in parsed_json:
                        items_array = parsed_json["items"]
                        # Convert items to JSON string and re-parse
                        items_json = json.dumps(items_array)
                        return parser.parse(items_json)
                    # Handle direct array format
                    elif isinstance(parsed_json, list):
                        items_json = json.dumps(parsed_json)
                        return parser.parse(items_json)
                    # Handle {"test_cases": [...]} or similar
                    elif isinstance(parsed_json, dict):
                        for key in ["test_cases", "cases", "scenarios", "data"]:
                            if key in parsed_json and isinstance(parsed_json[key], list):
                                items_json = json.dumps(parsed_json[key])
                                return parser.parse(items_json)
                    
                    # If we can't extract an array, re-raise the original error
                    raise parse_error
                except (json.JSONDecodeError, ValueError) as json_error:
                    # If JSON parsing fails, try to extract JSON from the response
                    print(f"Warning: JSON parsing failed, attempting to extract JSON from response: {json_error}")
                    # Try to find JSON-like content in the response
                    json_pattern = r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}'
                    json_matches = re.findall(json_pattern, response_content, re.DOTALL)
                    if json_matches:
                        # Try the largest match (likely the full JSON)
                        for match in sorted(json_matches, key=len, reverse=True):
                            try:
                                parsed = json.loads(match)
                                if isinstance(parsed, dict) and "items" in parsed:
                                    return parser.parse(json.dumps(parsed["items"]))
                                elif isinstance(parsed, list):
                                    return parser.parse(json.dumps(parsed))
                            except:
                                continue
                    # Re-raise the original parsing error if all fallbacks fail
                    raise parse_error
